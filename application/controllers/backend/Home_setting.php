...
class Home_setting extends CI_Controller
{
	...
	function index()
	{
		...
		$x['title_2'] = $data->home_title_2;
		$x['title_3'] = $data->home_title_3;
		$x['title_4'] = $data->home_title_4;
	...
	function update()
	{
		...
		$title2 = htmlspecialchars($this->input->post('title2', TRUE), ENT_QUOTES);
		$title3 = htmlspecialchars($this->input->post('title3', TRUE), ENT_QUOTES);
		$title4 = htmlspecialchars($this->input->post('title4', TRUE), ENT_QUOTES);
		...
		if (!empty($_FILES['img_heading']['name']) && !empty($_FILES['img_testimonial']['name'])) {
			...
			$this->setting_model->update_information($home_id, $caption1, $caption2, $caption3, $title1, $title2, $title3, $title4, $bg_heading, $bg_testimoni);
			...
			$this->setting_model->update_information_heading($home_id, $caption1, $caption2, $caption3, $title1, $title2, $title3, $title4, $bg_heading);
			$this->session->set_flashdata('msg', 'success');
			redirect('backend/home_setting');
		} elseif (empty($_FILES['img_heading']['name']) && !empty($_FILES['img_testimonial']['name'])) {
			if ($this->upload->do_upload('img_testimonial')) {
			...
			$this->setting_model->update_information_testimoni($home_id, $caption1, $caption2, $caption3, $title1, $title2, $title3, $title4, $bg_testimoni);
			$this->session->set_flashdata('msg', 'success');
			redirect('backend/home_setting');
		} else {
			$this->setting_model->update_information_noimg($home_id, $caption1, $caption2, $caption3, $title1, $title2, $title3, $title4);
			$this->session->set_flashdata('msg', 'success');
			redirect('backend/home_setting');
		}
	}
